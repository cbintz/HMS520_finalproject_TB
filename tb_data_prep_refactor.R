...
tb_funding_data <- Reduce(function(x, y) merge(x, y, by = c("iso3", "year"), all=TRUE), list(mdr_burden, tb_burden, tb_notifications, tb_lab_expenditure, pop_dens_long,health_funding_data_long,  private_funding_data_long,ihme_tb_funding  ))
...
intersect(colnames(locs), colnames(tb_funding_data))
tb_funding_data <- merge(locs, tb_funding_data, by.x = c("ihme_loc_id","location_name", "location_id"), by.y = c("iso3", "location_name", "location_id"))
